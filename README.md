...
The plugin registers the LetMeSendEmail MCP server at `https://mcp.letmesend.email` (streamable HTTP) with 35 tools. It authenticates via OAuth - supported clients walk you through sign-in on first connect. For headless use, a Bearer API key works instead:
